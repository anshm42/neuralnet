...
public:
    ...
    void addLayer(int in, int neurons, ActivationType actType = ActivationType::RELU) {
    ...
    void train(const std::vector<VectorXd>& data, const std::vector<VectorXd>& labels, 
               double learningRate, int batchSize, int epochs = 20, double decayRate = 0.8) {
        ...
        std::cout << "Learning rate decay: " << decayRate << " (every 5 epochs)\n";
        ...
        for (size_t i = 0; i < layers.size(); i++) {
            ...
            if (i < layers.size() - 1) {
                ...
                std::cout << layers[i].getWeights().rows() << " (" << actType << ") → " << layers[i].getWeights().cols() << "\n";
            }
        }
        std::cout << "=======================================\n\n";
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            if (epoch > 0 && epoch % 5 == 0) {
                lr *= decayRate;
                std::cout << "Learning rate reduced to: " << std::fixed << std::setprecision(6) << lr << "\n";
            }
            ...
            for (int batch = 0; batch < numBatches; batch++) {
                ...
                if (batch % progressStep == 0 || batch == numBatches - 1) {
                    ...
                    std::cout << "] " << progress << "% (" << (batch + 1) << "/" << numBatches << " batches)";
                    std::cout.flush();
                }
            }
            
            totalLoss /= numBatches;
            std::cout << "\nEpoch " << (epoch + 1) << "/" << epochs << " completed. Avg Loss: " 
                      << std::fixed << std::setprecision(6) << totalLoss << "\n";
            
            if (totalLoss < bestLoss) {
                bestLoss = totalLoss;
                std::cout << "New best loss: " << std::fixed << std::setprecision(6) << bestLoss << "\n";
            }
            ...
            if ((epoch + 1) % 2 == 0 || epoch == epochs - 1) {
                ...
                double validationAccuracy = test(validationData, validationLabels, true);
                std::cout << "Validation Accuracy: " << std::fixed << std::setprecision(2) << validationAccuracy << "%\n";
                
                if (validationAccuracy > bestAccuracy) {
                    bestAccuracy = validationAccuracy;
                    std::cout << "New best validation accuracy: " << std::fixed << std::setprecision(2) << bestAccuracy << "%\n";
                }
            }
        ...
        std::cout << "\n===== TRAINING COMPLETED =====\n";
        std::cout << "Final best loss: " << std::fixed << std::setprecision(6) << bestLoss << "\n";
        std::cout << "Best validation accuracy: " << std::fixed << std::setprecision(2) << bestAccuracy << "%\n";
        std::cout << "==============================\n\n";
    }

    double test(const std::vector<VectorXd>& data, const std::vector<VectorXd>& labels, bool isValidation = false) {
        int correct = 0;
        double totalLoss = 0.0;
        ...
        for (size_t i = 0; i < data.size(); i++) {
            ...
            if (!isValidation && (i % progressStep == 0 || i == data.size() - 1)) {
                ...
                std::cout << "] " << progress << "% (" << (i + 1) << "/" << data.size() << " samples)";
                std::cout.flush();
            }
        }
        ...
        if (!isValidation) {
            std::cout << "\n\n===== TESTING COMPLETED =====\n";
            std::cout << "Test set average loss: " << std::fixed << std::setprecision(6) << avgLoss << "\n";
            std::cout << "Test set accuracy: " << std::fixed << std::setprecision(2) << accuracy << "% (" 
